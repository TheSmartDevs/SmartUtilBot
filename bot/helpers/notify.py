...
async def check_channel_membership(bot: Bot, user_id: int) -> tuple[bool, str, Optional[int]]:
    try:
        ...
        result = await bot(GetChatMember(chat_id=channel_id, user_id=user_id))
        
        if result.status in [ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
