...
async def handle_topbin_command(message: Message, bot: Bot):
    LOGGER.info(f"Received /topbin command from user: {message.from_user.id if message.from_user else 'Unknown'} in chat {message.chat.id}")
    ...
    try:
        if not message.reply_to_message or not message.reply_to_message.document or not message.reply_to_message.document.file_name.endswith('.txt'):
            ...
            LOGGER.warning(f"Invalid reply for /topbin command by user: {message.from_user.id}")
            return
        ...
        if file_size_mb > MAX_TXT_SIZE:
            ...
            LOGGER.warning(f"File size exceeds limit for /topbin command by user: {message.from_user.id}")
            return
        ...
        if not top_bins:
            ...
            LOGGER.info(f"No BIN data found for /topbin command in chat {message.chat.id}")
            return
        ...
        LOGGER.info(f"Successfully sent top BINs to chat {message.chat.id}")

    except Exception as e:
        LOGGER.error(f"Error processing /topbin command in chat {message.chat.id}: {str(e)}")
