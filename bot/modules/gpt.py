...
async def fetch_gpt_response(prompt, model):
    if not OPENAI_API_KEY or OPENAI_API_KEY.strip() == "":
        ...
        await Smart_Notify(bot, "gpt", "OpenAI API key is missing or invalid", message)
        return None
        ...
        try:
            async with session.post(url, headers=headers, json=data) as response:
                if response.status == 200:
                    ...
                    await Smart_Notify(bot, "gpt", f"OpenAI API returned status {response.status}: {error_response}", message)
                    return None
